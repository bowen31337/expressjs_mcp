...
import { z } from "zod";
import { ApiOperation, Controller, Get, Post } from "../../src/decorators";
...
async function start() {
	...
	app.listen(PORT, () => {
		...
		console.log(
			`     -d '{"toolName": "GET_/stream/events", "args": {}, "streaming": true}'`,
		);
		...
		console.log(`     -H "Content-Type: application/json" \\`);
		console.log(
			`     -d '{"toolName": "POST_/stream/chat", "args": {"message": "Hello!"}, "streaming": true}'`,
		);
		...
		console.log(`     -H "Content-Type: application/json" \\`);
		console.log(
			`     -d '{"toolName": "GET_/stream/progress/:taskId", "args": {"taskId": "task-123"}, "streaming": true}'`,
		);
