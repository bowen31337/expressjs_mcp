...
app.post("/admin/tools/add", async (req: Request, res: Response) => {
  ...
  if (["get", "post", "put", "delete", "patch"].includes(methodLower)) {
    ...
    dynamicRoutes.set(routeKey, { path, method, handler: routeHandler, schema });
...
app.get("/admin/tools/list", (req: Request, res: Response) => {
  ...
  res.json({
    total: tools.length,
    static: tools.filter((t: any) => !dynamicToolsList.includes(t.title)).length,
...
app.post("/admin/tools/batch", async (req: Request, res: Response) => {
  ...
  for (const op of operations) {
    ...
    try {
      ...
      results.push({ route: routeKey, status: "error", error: (error as Error).message });
    }
  }
...
async function start() {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`   curl -X POST http://localhost:${PORT}/admin/tools/load-config \\`);
    console.log(`     -H "Content-Type: application/json" \\`);
    console.log(`     -d '{}'`);
