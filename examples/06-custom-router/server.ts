...
const loggingMiddleware = (req: Request, res: Response, next: NextFunction) => {
  ...
  res.on("finish", () => {
    console.log(`[LOG] ${req.method} ${req.path} - ${res.statusCode} - ${Date.now() - start}ms`);
  });
  next();
};
...
apiV2Router.get("/stream", (req: Request, res: Response) => {
  ...
  const interval = setInterval(() => {
    res.write(`data: ${JSON.stringify({ version: "2.0", count: ++counter })}\n\n`);
    if (counter >= 5) {
...
async function start() {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`   Admin MCP:  http://localhost:${PORT}/mcp/admin/tools (auth required)`);
    console.log(`   API v1 MCP: http://localhost:${PORT}/mcp/api/v1/tools (deprecated)`);
    ...
    console.log(`   Admin routes:   ${adminMcp.listTools().length} tools (protected)`);
    console.log(`   API v1 routes:  ${apiV1Mcp.listTools().length} tools (legacy)`);
    ...
    console.log(`   curl -X POST http://localhost:${PORT}/mcp/custom/invoke \\`);
    console.log(`     -H "Content-Type: application/json" \\`);
    console.log(`     -d '{"toolName": "GET_/public/products", "category": "public", "args": {}}'`);
  });
}
